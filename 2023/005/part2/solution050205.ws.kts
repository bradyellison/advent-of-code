...
//                print("  ")
                for (mapping in locationPath) {
                    val start = mapped
                    // solution #1 but we linear search for starting range before range check
//                    mapping.second.binarySearchBy(it) { it.fromStart }
                    val mappedRange = mapping.second
                            .findLast { mapped >= it.fromStart }
                            .takeIf { it?.fromRange?.contains(mapped) == true }
                    if (mappedRange != null) {
